...
const Answer = (props) => {
  ...
  const deleteAnswer = async function (id) {
    if (typeof window !== "undefined") {
      if (user) {
        const ref = doc(db, "text", props.question.docId, "answer", id);;
        const docSnap = await getDoc(ref);
        if (docSnap.exists()) {
          if (docSnap.data().userid == user.uid) {
            try {
              ...
              await deleteDoc(doc(db, "text", props.question.docId, "answer", id));
          ...
          <Box display="flex" flexDirection="column" sx={{ p: 2 }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" >
              <Box display="flex" flexDirection="row" alignItems="center">
                ...
                <Typography style={{ margin: 0, textAlign: "left" }}>{answer.user}</Typography>
              </Box>
              <Box display="flex" alignItems="center">
                ...
                >
                  {answer.heart ? count : 0}
                </Typography>
                {user && user.uid === comment.userid ? answerDelete(comment) : usual()}
              </Box>
            </Box>
            <Box>
              

              <Typography variant="body2" sx={{m:2, textAlign: "left" }}>{answer.content}</Typography>
              <Typography variant="caption" style={{ textAlign: "left", color: "grey" }}>
                {answer.timestamp &&
                  answer.timestamp.toDate().toLocaleString()}
              </Typography>
            </Box>
