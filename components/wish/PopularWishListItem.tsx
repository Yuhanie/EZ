...
const PopularWishListItem: React.FC<Props> = (props) => {
  const [open, setOpen] = useState(false);
  const [currentUser, setCurrentUser] = useState<User>();
  const [heartCount, setHeartCount] = useState(
    props.wish.heart ? props.wish.heart.length : 0
  );
      ...
      if (user) {
        ...
        if (docSnapshot.exists()) {
          setProfile({
            ...
            character: docSnapshot.data().character
              ? docSnapshot.data().character
              : "學習者",
            majortag: docSnapshot.data().majortag
              ? docSnapshot.data().majortag
              : [],
          });
        }
      ...
      >
        ...
        <Grid container>
          ...
          <Grid item display="flex" justifyContent="center" alignItems="center">
            <Grid item>{decide == true && solvedIcon()}</Grid>
            {decide == false && (
              <Grid item md={6}>
...
export default PopularWishListItem;
